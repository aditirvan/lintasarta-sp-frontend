        ...
        <v-card class="rounded-lg" flat>
          <v-container fluid class="pa-7" id="container">
            ...
            <v-row class="my-0 py-0" v-if="mode == 'https'">
              ...
              <v-col cols="12" md="6" class="my-0"  style="padding-top: 1.4%;" v-else>
                <div class="d-flex">
                  <p class="font-weight-bold my-0">Security File</p>
                  <v-spacer></v-spacer>
                  <!-- <v-btn large color="success" @click="addBe()">Add Backend -->
                  <v-btn
                    :disabled="isLoadingBtn"
                    color="success"
                    @click="changeSSL=!changeSSL"
                    large
                  >
                    Update
                  </v-btn>
                ...
                <v-text-field
                  ...
                  v-model="ssl_file"
                ></v-text-field>
                <p class="mb-2 fz-12 font-italic">
                  *Expired at {{ $moment(expiredAt).locale("en").format("DD/MM/YYYY hh:mm A") }}
                </p>
              </v-col>
            </v-row>
            <!-- untuk ganti ssl ketika edit -->
            <v-row class="my-0 py-0" v-if="changeSSL">
              <v-col cols="12" md="6" class="my-0">
                ...
                prepend-icon=""
                placeholder="Choose your file"
                v-model="ssl_file_edit"
                :error-messages="$v.ssl_file_edit.$errors.length?$v.ssl_file_edit.$errors[0].$message:''"
                ></v-file-input>
              </v-col>
            </v-row>
            <v-row class="my-0 py-0" v-if="changeSSL">
              <v-col cols="12" md="6" class="my-0">
                ...
                prepend-icon=""
                placeholder="Choose your file"
                v-model="key_file_edit"
                :error-messages="$v.key_file_edit.$errors.length?$v.key_file_edit.$errors[0].$message:''"
                ></v-file-input>
              </v-col>
            </v-row>
...
export default {
  ...
  setup(props, context) {
    ...
    const changeSSL = ref(false)
    const key_file_edit = ref(null)
    const ssl_file_edit = ref(null)
    ...
    const $v = useVuelidate(
      {
        ...
        ssl_file_edit: {
          $autoDirty: true,
          required: requiredIf(() => {return changeSSL.value}) ,
    ...
    const createFrontend = async () => {
      ...
      if(headline.value == 'Edit'){
        ...
        if (changeSSL.value) {
          let lengthName = ssl_file_edit.value.name.length
          let cert_name = ssl_file_edit.value.name.slice(0, lengthName -4)
          let cert_extension = ssl_file_edit.value.name.substr(ssl_file_edit.value.name.length -4)
          let doc_cert = await getBase64(ssl_file_edit.value)
          ...
          let lengthNameKey = key_file_edit.value.name.length
          let key_name = key_file_edit.value.name.slice(0, lengthNameKey -4)
          let key_extension = key_file_edit.value.name.substr(key_file_edit.value.name.length -4)
          let doc_key = await getBase64(key_file_edit.value)
      ...
      if(headline.value == 'Create'){
        ...
        if(resp.status == 200){
          context.root.$router.replace("/network/slb/config/frontend");
        }else{
          console.log(resp)
        }
      }else{
    ...
    onMounted(() => {
      ...
      if(edit){
        expiredAt.value = edit.expired_at
        let rulesItem = []
        if(edit.rules != null){
