<template>
  <div>
    <v-row>
      <v-col cols="12">
        <div v-if="getIsLoading" class="d-flex justify-center">
          <v-progress-circular
            :size="50"
            color="primary"
            indeterminate
        ...
        <div v-else>
            ...
            <v-container fluid>
                ...
                <div class="d-flex justify-end align-end pb-1" v-if="getActivePackageAddOns.length && !read_only">
                    ...
                    width="150"
                    @click=" () => {
                            validateprivilages(['Manage Service', 'editor', true]).then(() => {
                            modalUnsubs()
                            typePackage = 'addons'
                        })
                        }"
                    >Unsubscribe Add Ons</v-btn>
                    <v-btn 
                    color="secondary" 
            ...
            <v-row>
                ...
                <v-col class="d-flex justify-center" v-if="!read_only">
                    <v-btn color="secondary" link to="/overview/package-addons">
                        Purchase a Package
                    </v-btn>
                </v-col>
                </v-row>
...
export default {
  setup(props, context) {
        ...
        const { getActivePackageMonthly, getActivePackageAddOns, getIsLoading } = useNamespacedGetters(MANAGESERVICE, ["getActivePackageMonthly", "getActivePackageAddOns", "getIsLoading"])
