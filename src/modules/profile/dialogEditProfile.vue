<template>
  <v-dialog max-width="500" v-model="dialog">
    <v-card>
      ...
      <v-card-text>
        <p class="font-weight-bold">Full Name * <span class="red--text text-xs ml-1" style="font-size:0.75em">{{
            (!member.fullname || (member.fullname.length < 5)) ? errors : ''
          }}</span></p>
        <v-text-field
            maxlength="100"
            ...
            flat
            outlined
            :error-messages="!member.fullname || (member.fullname.length < 5) ? errors : ''"
        ></v-text-field>
      </v-card-text>
      <v-card-text>
        <p class="font-weight-bold">Phone Number * <span class="red--text text-xs ml-1" style="font-size:0.75em">{{
            !member.phone ? errors : ''
          }}</span></p>
        <v-text-field
      ...
      <v-card-text>
        <p class="font-weight-bold">Address * <span class="red--text text-xs ml-1"
                                                    style="font-size:0.75em">{{ !member.address ? errors : '' }}</span>
        </p>
        <v-text-field
            maxlength="100"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    function Update() {
      ...
      } else if (membervalue.fullname.length < 5) {
        errors.value = 'This field should be at least 5 long'
      } else {
        const Value = {
          id: membervalue.id,
